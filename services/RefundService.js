 ...
 */
const getRefundInfo = async (bookingId) => {
 ...
 */
const processRefund = async (bookingId, refundType, refundedByUserId, reason = null) => {
    const transaction = await sequelize.transaction();
    
    try {
        const refundInfo = await getRefundInfo(bookingId);
        ...
        if (refundType === 'stripe') {
            refundResult = await processStripeRefund(booking, stripeTransaction, transaction);
            refundAmount = stripeTransaction.amount; // Use original transaction amount in dollars
        } else {
            refundResult = await processCreditRefund(booking, creditUsage, refundedByUserId, transaction);
 ...
 * @private
 */
const processStripeRefund = async (booking, stripeTransaction, transaction) => {
 ...
 * @private
 */
const processCreditRefund = async (booking, creditUsage, refundedByUserId, transaction) => {
 ...
 */
const isEligibleForAutomaticRefund = (booking) => {
 ...
 */
const processAutomaticRefund = async (bookingId, studentId) => {
    const refundInfo = await getRefundInfo(bookingId);
    
    if (!isEligibleForAutomaticRefund(refundInfo.booking)) {
