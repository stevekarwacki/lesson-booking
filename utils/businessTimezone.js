...
const getTimezone = async () => {
    ...
    try {
        ...
        // Cache the result
        cachedTimezone = businessTz || DEFAULT_TIMEZONE;
        cacheExpiry = now + CACHE_DURATION;
        
        return cachedTimezone;
    } catch (error) {
        console.error('Error getting business timezone:', error);
        return DEFAULT_TIMEZONE; // Safe fallback
    }
};
 ...
 */
const toBusinessTimeString = async (date, options = {}) => {
 ...
 */
const toBusinessDateString = async (date) => {
    const timezone = await getTimezone();
 ...
 */
const convertTimezone = async (date, fromTimezone, toTimezone = null) => {
    const targetTimezone = toTimezone || await getTimezone();
